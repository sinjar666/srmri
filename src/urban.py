...
# Application for urban side of classico
...
gladefile = os.path.join(os.path.dirname(__file__),"urban.glade")
...
class FsUIPipeline:
    ...
    def try_rec_start(self):
        #print self.AUDIOREADY
        if self.VIDREADY:
            self.rec_queue = gst.element_factory_make("queue")
            self.pipeline.add(self.rec_queue)
            self.rec_colorspace = gst.element_factory_make("ffmpegcolorspace")
            self.pipeline.add(self.rec_colorspace)
            self.ffenc_mpeg4 = gst.element_factory_make("ffenc_mpeg4")
            self.pipeline.add(self.ffenc_mpeg4)
            self.matroskamux = gst.element_factory_make("matroskamux")
            self.pipeline.add(self.matroskamux)
            self.localsrc = gst.element_factory_make("gconfaudiosrc")
            self.pipeline.add(self.localsrc)
            self.convert3 = gst.element_factory_make("audioconvert")
            self.pipeline.add(self.convert3)
            self.ffenc_aac = gst.element_factory_make("ffenc_aac")
            self.pipeline.add(self.ffenc_aac)
            self.rec_queue2 = gst.element_factory_make("queue")
            self.pipeline.add(self.rec_queue2)
            self.rec_queue3 = gst.element_factory_make("queue")
            self.pipeline.add(self.rec_queue3)
            self.filesink = gst.element_factory_make("filesink")
            self.filesink.set_property("location",datetime.datetime.now().isoformat()+".mkv")
            self.pipeline.add(self.filesink)
            
            self.videosource.tee.link(self.rec_queue)
            self.rec_queue.link(self.rec_colorspace)
            self.rec_colorspace.link(self.ffenc_mpeg4)
            self.localsrc.link(self.convert3)
            self.convert3.link(self.ffenc_aac)
            self.ffenc_aac.link(self.rec_queue2)
            self.ffenc_mpeg4.link(self.rec_queue3)
            self.rec_queue2.link(self.matroskamux)
            self.rec_queue3.link(self.matroskamux)
            self.matroskamux.link(self.filesink)

            self.rec_queue.set_state(gst.STATE_PLAYING)
            self.localsrc.set_state(gst.STATE_PLAYING)
            self.rec_colorspace.set_state(gst.STATE_PLAYING)
            self.ffenc_mpeg4.set_state(gst.STATE_PLAYING)
            self.ffenc_aac.set_state(gst.STATE_PLAYING)
            self.convert3.set_state(gst.STATE_PLAYING)
            self.rec_queue2.set_state(gst.STATE_PLAYING)
            self.rec_queue3.set_state(gst.STATE_PLAYING)
            self.matroskamux.set_state(gst.STATE_PLAYING)
            self.filesink.set_state(gst.STATE_PLAYING)
...
class FsUIVideoSource(FsUISource):
    ...
    def make_source(self):
        ...
        filter.set_property("caps", gst.Caps("video/x-raw-yuv , width=320 , height=240, framerate=[20/1,30/1]"))
        bin.add(filter)
        source.link(filter)
...
class FsUIParticipant:
    ...
    def make_widget(self):
        ...
        #self.vidwindow = self.glade.get_widget("main_window")
        self.remoteframe = self.mainui.glade.get_widget("remote_frame")
        self.mainui.glade.signal_connect("exposed_r",self.exposed)
        #self.vidwindow.show()
        ...
        self.mainui.glade.signal_connect("on_full_b_clicked",self.on_full_b_clicked)
    ...
    def have_size(self, pad, buffer):
        ...
        y = buffer.caps[0]["height"]
        gtk.gdk.threads_enter()
        self.mainui.glade.get_widget("remote_drawingarea").set_size_request(int(x*1.5),int(y*1.5))
        gtk.gdk.threads_leave()
    ...
    def destroy(self):
        if VIDEO:
            try:
                ...
                pass
            self.mainui.glade.get_widget("remote_drawingarea").disconnect_by_func(self.exposed)
            sys.exit(1)
            self.streams = {}
            self.outcv.acquire()
    ...
    def on_full_b_clicked(self,data):
        if not self.isfull:
            ...
            hbox1 = self.mainui.glade.get_widget("hbox1")
            self.remoteframe.reparent(hbox1)
...
class UrbanMainUi:
    ...
    def __init__(self,ip,port):
        ...
        #since server mode
        self.server = FsUIListener(port, FsUIServer, mycname, FsUIParticipant, self.pipeline, self)
        self.cname_label=self.glade.get_widget("cname_l")
        self.cname_label.set_markup("<b>%s</b>\nExpecting connections on port %s\n\nUsing codec:\nAUDIO\nPCMA sampled at 8000 bits/s\n\nVIDEO\nH.263-1998 sampled at 90000 bits/s\n" % (mycname, self.server.port))
        ...
        self.glade_float = gtk.glade.XML(gladefile, "float_window")
        self.glade_float.signal_connect("alert_dtmf_start_cb",self.alert_dtmf_start_cb)
        self.glade_float.signal_connect("alert_dtmf_stop_cb",self.alert_dtmf_stop_cb)
        self.floatwindow = self.glade_float.get_widget("float_window")
    ...
    def reset_codecs(self,fssession):
        ...
        for c in fssession.get_property("codecs"):
            ...
            if current and c and current.id==c.id:
                ...
                #self.cname_label.set_markup(self.cname_label.get_label() + "\nactive: " + c.to_string() + "\n")
    ...
    def newsize (self, x, y):
        self.glade.get_widget("preview_drawingarea").set_size_request(int(x*0.55),int(y*0.55))
